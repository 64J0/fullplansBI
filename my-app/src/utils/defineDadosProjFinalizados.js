export default function defineDadosASeremMostrados(
  opcaoInputRadio,
  opcaoAnoSelect,
  opcaoPessoaSelect,
  dadosFiltrados
  ...
  if (opcaoInputRadio === "mes") {
    ...
    if (opcaoPessoaSelect === "Todas as pessoas") {
      if (opcaoAnoSelect === "Todos os anos") {
        ...
        dadosFiltrados.map((dado) => {
          ...
          return objMostrar.y[indice]++; // Incrementa o valor salvo na posição do mês
        });
      } else {
        // funcionando!!
        dadosFiltrados.map((dado) => {
      ...
      if (opcaoAnoSelect === "Todos os anos") {
        dadosFiltrados.map((dado) => {
          dado.infoProjetos.map((infoProjeto) => {
            if (
              String(infoProjeto.projetistaDesenho) ===
              String(opcaoPessoaSelect)
            ) {
    ...
    if (opcaoPessoaSelect === "Todas as pessoas") {
      ...
      dadosFiltrados.map((dado) => {
        ...
        dado.infoProjetos.map((infoProjeto) => {
          if (
            String(infoProjeto.projetistaDesenho) === String(opcaoPessoaSelect)
          ) {
